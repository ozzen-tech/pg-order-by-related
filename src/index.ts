...
import { LIB_NAME, LIB_VERSION } from "./pkg.js";
...
const PgOrderByRelatedPlugin: Plugin = (
  ...
  builder.hook("build", (build) => {
    ...
    if (!build.versions) {
      throw new Error(
        `Plugin ${LIB_NAME}@${LIB_VERSION} requires graphile-build@^4.1.0 in order to check dependencies (current version: ${build.graphileBuildVersion})`
      );
    }
    const depends = (name: string, range: string) => {
      if (!build.hasVersion(name, range)) {
        throw new Error(
          `Plugin ${LIB_NAME}@${LIB_VERSION} requires ${name}@${range} (${
            build.versions[name]
    ...
    build.versions = build.extend(build.versions, { [LIB_NAME]: LIB_VERSION });
